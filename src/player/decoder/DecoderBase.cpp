...
void DecoderBase::SeekToPosition(float position) {
    printf("DecoderBase::SeekToPosition position=%f", position);
    std::unique_lock<std::mutex> lock(m_Mutex);
...
int DecoderBase::Init(const char *url, AVMediaType mediaType) {
    printf("DecoderBase::Init url=%s, mediaType=%d", url, mediaType);
    strcpy(m_Url, url);
...
void DecoderBase::UnInit() {
    printf("DecoderBase::UnInit m_MediaType=%d", m_MediaType);
    if (m_Thread) {
        Stop();
        m_Thread->join();
        delete m_Thread;
    ...
    printf("DecoderBase::UnInit end, m_MediaType=%d", m_MediaType);
}
...
int DecoderBase::InitFFDecoder() {
    ...
    do {
        ...
        if (avformat_open_input(&m_AVFormatContext, m_Url, NULL, NULL) != 0) {
            printf("DecoderBase::InitFFDecoder avformat_open_input fail.");
            break;
        }

        //3.获取音视频流信息
        if (avformat_find_stream_info(m_AVFormatContext, NULL) < 0) {
            printf("DecoderBase::InitFFDecoder avformat_find_stream_info fail.");
            break;
        }
        ...
        if (m_StreamIndex == -1) {
            printf("DecoderBase::InitFFDecoder Fail to find stream index.");
            break;
        }
        //5.获取解码器参数
        ...
        if (m_AVCodec == nullptr) {
            printf("DecoderBase::InitFFDecoder avcodec_find_decoder fail.");
            break;
        }
        ...
        if (avcodec_parameters_to_context(m_AVCodecContext, codecParameters) != 0) {
            printf("DecoderBase::InitFFDecoder avcodec_parameters_to_context fail.");
            break;
        }
        ...
        if (result < 0) {
            printf("DecoderBase::InitFFDecoder avcodec_open2 fail. result=%d", result);
            break;
        }
        result = 0;
...
void DecoderBase::UnInitDecoder() {
    printf("DecoderBase::UnInitDecoder");
    if (m_Frame != nullptr) {
...
void DecoderBase::DecodingLoop() {
    printf("DecoderBase::DecodingLoop start, m_MediaType=%d", m_MediaType);
    {
        std::unique_lock<std::mutex> lock(m_Mutex);
        m_DecoderState = STATE_DECODING;
        lock.unlock();
    ...
    for (;;) {
        while (m_DecoderState == STATE_PAUSE) {
            std::unique_lock<std::mutex> lock(m_Mutex);
            printf("DecoderBase::DecodingLoop waiting, m_MediaType=%d", m_MediaType);
    ...
    printf("DecoderBase::DecodingLoop end");
}

void DecoderBase::UpdateTimeStamp() {
    printf("DecoderBase::UpdateTimeStamp");
    std::unique_lock<std::mutex> lock(m_Mutex);
...
//        printf("DecoderBase::AVSync m_CurTimeStamp=%ld, elapsedTime=%ld", m_CurTimeStamp, elapsedTime);
//
//        if(m_CurTimeStamp > elapsedTime) {
//            //休眠时间
//            auto sleepTime = static_cast<unsigned int>(m_CurTimeStamp - elapsedTime);//ms
...
int DecoderBase::DecodeOnePacket() {
    printf("DecoderBase::DecodeOnePacket m_MediaType=%d", m_MediaType);
    if (m_SeekPosition > 0) {
        //seek to frame
        ...
        if (seek_ret < 0) {
            m_SeekSuccess = false;
            printf("BaseDecoder::DecodeOneFrame error while seeking m_MediaType=%d", m_MediaType);
        } else {
            if (-1 != m_StreamIndex) {
            ...
            ClearCache();
            m_SeekSuccess = true;
            printf("BaseDecoder::DecodeOneFrame seekFrame pos=%f, m_MediaType=%d", m_SeekPosition, m_MediaType);
        }
    }
...
//            }
            ...
            while (avcodec_receive_frame(m_AVCodecContext, m_Frame) == 0) {
                ...
                AVSync();
                //渲染
                printf("DecoderBase::DecodeOnePacket 000 m_MediaType=%d", m_MediaType);
                OnFrameAvailable(m_Frame);
                printf("DecoderBase::DecodeOnePacket 0001 m_MediaType=%d", m_MediaType);
                frameCount++;
            }
            printf("BaseDecoder::DecodeOneFrame frameCount=%d", frameCount);
            //判断一个 packet 是否解码完成
            if (frameCount > 0) {
                result = 0;
                goto __EXIT;
...
void DecoderBase::DoAVDecoding(DecoderBase *decoder) {
    printf("DecoderBase::DoAVDecoding");
    do {

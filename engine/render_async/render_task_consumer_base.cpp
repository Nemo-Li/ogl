...
void RenderTaskConsumerBase::CreateTexImage2D(RenderTaskBase *task_base) {
    ...
    //3. 将图片rgb数据上传到GPU;
    glTexImage2D(GL_TEXTURE_2D, 0, task->gl_texture_format_, task->width_, task->height_, 0, task->client_format_,
                 task->data_type_, task->data_);
    __CHECK_GL_ERROR__
...
void RenderTaskConsumerBase::DeleteTextures(RenderTaskBase *task_base) {
    ...
    glDeleteTextures(task->texture_count_, texture_id_array);
    __CHECK_GL_ERROR__
...
void RenderTaskConsumerBase::UpdateTextureSubImage2D(RenderTaskBase *task_base) {
    ...
    __CHECK_GL_ERROR__
    glTexSubImage2D(GL_TEXTURE_2D, 0, task->x_, task->y_, task->width_, task->height_, task->client_format_,
                    task->data_type_, task->data_);
    __CHECK_GL_ERROR__
}
...
void RenderTaskConsumerBase::CreateVAO(RenderTaskBase *task_base) {
    ...
    __CHECK_GL_ERROR__
    GLint attribute_normal_location = glGetAttribLocation(shader_program, "a_normal");
    __CHECK_GL_ERROR__
    ...
    __CHECK_GL_ERROR__
    {
        ...
        glVertexAttribPointer(attribute_pos_location, 3, GL_FLOAT, false, task->vertex_data_stride_, 0);
        __CHECK_GL_ERROR__
        //启用顶点Shader属性(a_color)，指定与顶点颜色数据进行关联
        if (attribute_color_location >= 0) {
            glVertexAttribPointer(attribute_color_location, 4, GL_FLOAT, false, task->vertex_data_stride_,
                                  (void *) (sizeof(float) * 3));
            __CHECK_GL_ERROR__
        }
        ...
        if (attribute_normal_location >= 0) {
            glVertexAttribPointer(attribute_normal_location, 3, GL_FLOAT, false, task->vertex_data_stride_,
                                  (void *) (sizeof(float) * (3 + 4 + 2)));
            __CHECK_GL_ERROR__
        }

        glEnableVertexAttribArray(attribute_pos_location);
        __CHECK_GL_ERROR__
        if (attribute_color_location >= 0) {
...
void RenderTaskConsumerBase::UpdateVBOSubData(RenderTaskBase *task_base) {
    ...
    __CHECK_GL_ERROR__
    //更新Buffer数据
    glBufferSubData(GL_ARRAY_BUFFER, 0, task->vertex_data_size_, task->vertex_data_);
    __CHECK_GL_ERROR__
}
...
void RenderTaskConsumerBase::SetEnableState(RenderTaskBase *task_base) {
    ...
    if (task->enable_) {
        glEnable(task->state_);
        __CHECK_GL_ERROR__
    } else {
        glDisable(task->state_);
        __CHECK_GL_ERROR__
    }
}
...
void RenderTaskConsumerBase::SetBlendFunc(RenderTaskBase *task_base) {
    ...
    glBlendFunc(task->source_blending_factor_, task->destination_blending_factor_);
    __CHECK_GL_ERROR__
}
...
void RenderTaskConsumerBase::SetUniformMatrix4fv(RenderTaskBase *task_base) {
    ...
    __CHECK_GL_ERROR__
    glUniformMatrix4fv(uniform_location, 1, task->transpose_ ? GL_TRUE : GL_FALSE, &task->matrix_[0][0]);
    __CHECK_GL_ERROR__
}
...
void RenderTaskConsumerBase::SetUniform1f(RenderTaskBase *task_base) {
    ...
    GLint uniform_location = glGetUniformLocation(shader_program, task->uniform_name_);
    __CHECK_GL_ERROR__
    glUniform1f(uniform_location, task->value_);
    __CHECK_GL_ERROR__
}
...
void RenderTaskConsumerBase::SetUniform3f(RenderTaskBase *task_base) {
    ...
    GLint uniform_location = glGetUniformLocation(shader_program, task->uniform_name_);
    __CHECK_GL_ERROR__
    glUniform3f(uniform_location, task->value_.x, task->value_.y, task->value_.z);
    __CHECK_GL_ERROR__
}
...
void RenderTaskConsumerBase::BindVAOAndDrawElements(RenderTaskBase *task_base) {
    ...
    __CHECK_GL_ERROR__
    {
        glDrawElements(GL_TRIANGLES, task->vertex_index_num_, GL_UNSIGNED_SHORT, 0);
...
void RenderTaskConsumerBase::SetClearFlagAndClearColorBuffer(RenderTaskBase *task_base) {
    ...
    glClear(task->clear_flag_);
    __CHECK_GL_ERROR__
    glClearColor(task->clear_color_r_, task->clear_color_g_, task->clear_color_b_, task->clear_color_a_);
    __CHECK_GL_ERROR__
}
...
void RenderTaskConsumerBase::SetStencilFunc(RenderTaskBase *task_base) {
    ...
    glStencilFunc(task->stencil_func_, task->stencil_ref_, task->stencil_mask_);
    __CHECK_GL_ERROR__
}
...
void RenderTaskConsumerBase::SetStencilOp(RenderTaskBase *task_base) {
    ...
    glStencilOp(task->fail_op_, task->z_test_fail_op_, task->z_test_pass_op_);
    __CHECK_GL_ERROR__
}
...
void RenderTaskConsumerBase::SetStencilBufferClearValue(RenderTaskBase *task_base) {
    ...
    glClearStencil(task->clear_value_);
    __CHECK_GL_ERROR__
}
